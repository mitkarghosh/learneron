...
<div class="body-section">
	<section class="top-question-section search-question">
		<div class="container">
			<div class="row">
				<div class="col-md-9 col-sm-8 question-left"><a id="perpage"></a><a id="mostviewed_question"></a>
				...
				<?php } ?>
					<div class="question-nav clearfix">
						<ul>
							...
							<li class="active"><a href="<?php echo Router::url(['controller'=>'Questions','action'=>'most-viewed-questions']);?>">Most Viewed</a></li>
				...
				<?php
				if(!empty($most_viewed_questions)){
				...
				<?php if(!empty($most_viewed_questions)){ ?>
					<div class="loader">
...
$(function(){
	$(document).on('click', 'a[rel="next"]', function() {
		...
		var target = $(this).attr('href').replace("most-viewed-questions","mostviewed_questions_search");
...
$('#no_of_questions').on('change', function(){
	...
	<?php if(@$this->request->query('search') && $this->request->query('search') !== NULL){ ?>
		if(this.value != ''){
			url = '<?php echo Router::url(NULL,true).'?search='.$this->request->query('search').'&limit=';?>'+this.value+'#mostviewed_question';
		}else{
			url = '<?php echo Router::url(NULL,true).'?search='.$this->request->query('search');?>#mostviewed_question';
		}
	<?php }else{ ?>
		if(this.value != ''){
			url = '<?php echo Router::url(NULL,true).'?limit=';?>'+this.value+'#mostviewed_question';
		}else{
