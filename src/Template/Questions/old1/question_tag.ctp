			...
			<div class="row">
				<div class="col-md-9 col-sm-8 question-left"><a id="perpage"></a>
					<div class="question-nav clearfix">
						<ul>
							<li class="active"><a href="javascript:void(0);"><?php echo $tagdata['title'];?></a></li>							
						</ul>
...
$(function(){
	$(document).on('click', 'a[rel="next"]', function() {
		...
		var target = $(this).attr('href').replace("question-tag","questiontag_search");
...
$('#no_of_questions').on('change', function(){
	var url = '<?php echo Router::url(array('controller'=>'','action'=>'tag/'.$tagdata['slug'].'?limit='));?>'+this.value+'#perpage';
